...
class MobRobCtrlAlg2():
	...
	def step(self, pose):	
		...
		if (self.k >= len(self.path)):
			return [0,0]
		local_goal= self.path[self.k]
		teta = pose[2]
		Dx = pose[0]-self.path[self.k][0]
		Dy = pose[1]-self.path[self.k][1]
		self.k_rho = np.sqrt(Dx**2 + Dy**2)
		self.k_alpha = -teta + np.arctan2(Dy,Dx)
		beta = -teta - self.k_alpha
		sigma = -1
		if (self.k_alpha > -np.pi/2 and self.k_alpha<= np.pi/2):
			sigma = 1
		v = self.k_rho
		w = -self.k_alpha
		uD = (2*v + w*self.wheel_distance) / 2*self.wheel_radius
		uL = 2*v/self.wheel_radius - uD 
		u = [uD, uL]
		# print(u)
		self.k = self.k + 2
...
def task6(map_file_name, path, t_sim):
	# Scene
	s = vis.visualizer()

	# Map.
	set_map(s, map_file_name, [25, 25])

	# Robot.
	ctrlalg = MobRobCtrlAlg2(path)

	# Mobile robot simulation and animation.	
	traj = mobrobsimanimate(s, np.array([path[0,0], path[0,1], 0.0]), ctrlalg, t_sim, 0.1)
...
def main():
	...
	#task3()
	#task4()
	path = task5('map.png', np.array([230,125]), np.array([125, 125]), 1, 5)
	...
	for i in range(len(path)):
			path[i]= np.array([path[i,0]/10, path[i,1]/10])

	# print(path)
	task6('map.png', path, 1)
